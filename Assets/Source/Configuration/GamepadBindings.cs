...
namespace Assets.Source.Configuration
{
    ...
    public class GamepadBindings : ConfigurationBase, IBindings
    {
        public Dictionary<string, KeyCodeValue> Bindings { get; set; } = new Dictionary<string, KeyCodeValue>()
        {
            ...
            { InputConstants.K_MOVE_DOWN,       new KeyCodeValue(GamepadConstants.GP_LEFTSTICK_V, KeyCodeValue.AxisDirections.Positive) },
            { InputConstants.K_MOVE_UP,         new KeyCodeValue(GamepadConstants.GP_LEFTSTICK_V, KeyCodeValue.AxisDirections.Negative) },
